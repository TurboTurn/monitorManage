...
@Component
@ServerEndpoint("/globalWebsocket")//库区总体监控的连接
public class GlobalWebSocket {// 仪表盘数据推送，该类实例为多例，每个socket连接都产生一个实例

	private Logger logger = LoggerFactory.getLogger(GlobalWebSocket.class);

	//concurrent包的线程安全Set，用来存放每个客户端对应的WebSocket对象。
	// 若要实现服务端与单一客户端通信的话，可以使用Map来存放，其中Key可以为用户标识
	public static CopyOnWriteArraySet<GlobalWebSocket> globalWebSockets = new CopyOnWriteArraySet<>();
	...
	public void onOpen(Session session) {//todo 第一次需要从数据库查询last()最新的数据，填充到仪表板，接下来只需要等待更新推送
		...
		globalWebSockets.add(this);     //加入set中
	...
	@OnClose
	public void onClose(){
		globalWebSockets.remove(this);  //从set中删除
		logger.info("socket连接关闭！当前连接数{}", globalWebSockets.size());
	}
	/**
	 * 收到客户端消息后调用的方法
